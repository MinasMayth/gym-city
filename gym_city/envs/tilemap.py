...
def zoneFromInt(i):
    zone_bps = [('Land', None), ('Water', 1), ('Forest', 22), ('Rubble', 44),
                ...
                ('Net', 844), ('Industrial', 852), ('Rubble', 860), ('Industrial', 888), ('CoalPowerPlant', 916),
                ('Stadium', 932), ('Bridge', 948),
...
class TileMap(object):
    ...
    def __init__(self, micro, MAP_X, MAP_Y, walker=False, paint=True):
        ...
        self.zoneSize = {'Residential': 3,
                         ...
                         'Church': 3,
                         'Hospital': 3,
                         'Radioactive': 1,  # Samy: why were this and the following line commented out?
                         'Flood': 1,
                         'Fire': 1
                         }
        ...
        self.road_labels = list(
            range(1, int(self.MAP_X * self.MAP_Y / 2) + 1))  # for potential one-hot encoding, for ex.,
    ...
    def setRoadNet(self, x, y, net_n):
        ...
        self.road_networks[0, x, y] = net_n
        self.road_net_sizes[net_n] += 1
        x0, y0, x1, y1 = max(0, x - 1), max(0, y - 1), min(x + 1, self.MAP_X - 1), min(y + 1,
                                                                                       self.MAP_Y - 1)  # get adj. coords
        for xi, yi in [(x0, y), (x1, y), (x, y0), (x, y1)]:
            if xi == x and yi == y:
                pass
                ...
                if net_i == 0:  # in the process of deleting, we have recursed bacck to the piece being deleted
                    ...
                    print("found unamissilated net piece while setting it to a new value", self.road_networks, x, y,
                          old_net, net_n, net_i)
                    raise Exception
    ...
    def addZoneBot(self, x, y, tool, static_build=False):
        ...
        if (zone in self.zone_compat and (old_zone in self.zone_compat[zone] or (
                old_zone in self.composite_zones and zone in self.composite_zones[old_zone]))) and zone != 'Water':
    ...
    def updateTile(self, x, y, zone=None, center=None, static_build=None):
        ...
        was_plant = (self.zoneMap[self.zoneInts['NuclearPowerPlant']][x][y] == 1) or (
                    self.zoneMap[self.zoneInts['CoalPowerPlant']][x][y] == 1)
        ...
        is_plant = (self.zoneMap[self.zoneInts['NuclearPowerPlant']][x][y] == 1) or (
                    self.zoneMap[self.zoneInts['CoalPowerPlant']][x][y] == 1)
    ...
    def getMapState(self):
        if self.walker:
            ...
            self.walkerZoneMap[:, self.MAP_X // 2: self.MAP_X + self.MAP_X // 2,
            self.MAP_Y // 2: self.MAP_Y + self.MAP_Y // 2] = self.zoneMap[:-1]
